...
public class GamePanel extends JPanel implements ActionListener {
    ...
    public void draw(Graphics g) {
        ...
        if (running) {
            //Paint rows ands coluns with index
            /*g.setColor(Color.WHITE);
            System.out.println(SCREEN_WIDTH/UNIT_SIZE);
            for(int i=0;i<SCREEN_WIDTH/UNIT_SIZE;i++) {
                g.drawLine(0, i*UNIT_SIZE, SCREEN_WIDTH, i*UNIT_SIZE);
                for(int x = 0;x<SCREEN_WIDTH/UNIT_SIZE;x++){
                    int index = (i*(SCREEN_WIDTH/UNIT_SIZE))+(x+1);
                    g.drawLine(x*UNIT_SIZE, 0, x*UNIT_SIZE, SCREEN_HEIGHT);
                    g.drawString(Integer.toString(index), (x*UNIT_SIZE)+3, (i*UNIT_SIZE)+g.getFont().getSize());
                }
            }*/
            //Paint fruit
